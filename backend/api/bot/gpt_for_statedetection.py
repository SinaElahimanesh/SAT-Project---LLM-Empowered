...
def if_data_sufficient_for_state_change(prompt_path, responses):
    ...
    with open("backend/api/bot/Information_prompts/Judge_prompt.md", "r", encoding="utf-8") as file:
        judge_prompt = file.read()

    with open(f"backend/api/bot/Information_prompts/{prompt_path}", "r", encoding="utf-8") as file:
        prompt = file.read()
